...
Function test
	...
	init()
	For i = 2 To 0 Step -1
		Move InfeedBlock /1 CP
		Move InfeedToken /1 CP
...
Function Pick_Infeed_Block
	...
	Go InfeedBlock +Z(Blocks * BlockHeight + InfeedHeight) /1
	Move InfeedBlock +Z(Blocks * BlockHeight) /1
	On 8
	Wait PressureTime
	Move InfeedBlock +Z(1 + (Blocks * BlockHeight)) /1
	Move InfeedBlock +Z(Blocks * BlockHeight + InfeedHeight) /1
...
Function Alignment_Block
    ...
    Go AlignBlock +Z(50) /1
	Move AlignBlock /1
	Off 8
	Move AlignBlock +X(2) +Y(-6) /1
	Move AlignBlock +Z(5) /1
	Move AlignBlock /1
	On 8
	Wait PressureTime
	Move AlignBlock +Z(20) /1
Fend

Function Place_Tray_Block
	'Tray Block
	Print "Placing Block in Tray. Block Position ID = ", Blocks
	Go TrayBlock +X(Blocks * TrayDistance) +Z(30) /1
...
Function Pick_Infeed_Token
	...
	Go InfeedToken +Z(Tokens * TokenHeight + InfeedHeight) /1
	Move InfeedToken +Z(Tokens * TokenHeight) /1
	On 8
	Wait PressureTime
	Move InfeedToken +Z(1 + (Tokens * TokenHeight)) /1
	Move InfeedToken +Z(Tokens * TokenHeight + InfeedHeight) /1
...
Function Alignment_Token
    ...
    Go AlignToken +Z(50) /1
	Move AlignToken /1
	Off 8
	Move AlignToken +X(5) /1
	Move AlignToken +Z(5) /1
	Move AlignToken /1
	On 8
	Wait PressureTime
	Move AlignToken +Z(20) /1
Fend

Function Place_Tray_Token
	'Tray Block
	Print "Placing Token in Tray. Token Position ID = ", Tokens
	Go TrayToken +X(Tokens * TrayDistance) +Z(30) /1
